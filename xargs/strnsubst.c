 ...
 * replstr as many time as can bedone before the constructed string is
 * maxsize bytes large. It does not freethe string pointed ty by str, it
...
strnsubst(char **str, const char *match, const char *replstr, size_t maxsize)
{
   ...
   * to duplicate s1. XXX we maybe should double-check wheither the match
  ...
  for (;;) {
    ...
    if ((strlen(s2) + strlen(s1) + strlen(replstr) - strlen(match) + 1) > maxsize) {
