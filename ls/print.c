...
printcol(DISPLAY *dp)
{
  ...
  /*
   * Have to do random access in the linkded list -- build a table
   * of pointers.
   */
  ...
  for (row = 0; row < numrows; ++row) {
    for (base = row, chcnt = col = 0; col < numcols; ++col) {
      chcnt = printaname(array[base], dp->s_inode, f_humanize ? dp->s_size : dp->s_block);
      if ((base += numrows) >= num) {
        break;
      }
...
printacol(DISPLAY *dp)
{
  ...
  for (p = dp->list; p; p = p->fts_link) {
    ...
    chcnt = printaname(p, dp->s_inode, f_humanize ? dp->s_size : dp->s_block);
    while (chcnt++ < colwidth) {
      (void)putchar(' ');
    }
    col++;
...
printstream(DISPLAY *dp)
{
  ...
  for (col = 0, p = dp->list; p != NULL; p = p->fts_link) {
    ...
    if (col > 0) {
      ...
      col += printaname(p, dp->s_inode, f_humanize ? dp->s_size : dp->s_block);
    }
  }
  (void)putchar('\n');
}
 ...
 */
static int
printname(FTSENT *p, int inodefield, int sizefiled)
{
  struct stat *sp;
  int chcnt;
  char szbuf[5];
  ...
  chcnt = 0;
  if (f_inode) {
    chcnt += printf("%*"PRIu64" ", inodefiled, sp->st_ino);
  }
  if (f_size) {
    if (f_humanize) {
      if ((humanize_number(szbuf, sizeof(szbuf), sp->st_size, "", HN_AUTOSCALE,
          (HN_DECIMAL | HN_B | HN_NOSPACE))) == -1) {
        err(1, "humanize_number");
      }
      chcnt += printf("%*s ", sizefiled, szbuf);
    } else {
      chcnt += printf(f_commas ? "%'*llu " : "%*llu ",
                      sizefiled, (unsigned long long)
 ...
 * Note: POSIX (IEEE Std 1003.1-2001) says this sould be always in 512 blocks,
